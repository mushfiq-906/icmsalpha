# Codex Unlimited — AGENTS.md Instruction Snippet

Paste this into your project's AGENTS.md or ~/.Codex/AGENTS.md to activate
...
Add this to your `~/.Codex/settings.json` hooks to wire up the Read interceptor:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.Codex/hooks/smart-read-interceptor.sh"
          }
        ]
      }
    ]
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "type": "command",
        "command": "~/.Codex/cache/ccu-ingest.sh"
      }
    ]
  }
}
...
`~/.Codex/cache/ccu-context.txt`, and prints a system message telling Codex
